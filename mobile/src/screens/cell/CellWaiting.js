...
export default function CellWaitingScreen({ navigation, route }) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (request?.accepted_at) {
        setElapsed(Math.floor((Date.now() - new Date(request.accepted_at)) / 1000));
      }
    }, 1000);
  ...
  useEffect(() => {
    ...
    socket.on('request_completed', () => {
      ...
      Alert.alert('Task Complete', 'The forklift task has been completed.',
        [{ text: 'OK', onPress: () => navigation.navigate('CellHome') }]
      );
    });

    // Handle all cancellation reasons including no forklifts
    socket.on('request_cancelled', (data) => {
      ...
      else if (reason === 'no_forklifts_available_offline') message = 'Forklift went offline. Please try again.';
  ...
  const handleCancel = () => {
    Alert.alert('Cancel Request', 'Are you sure you want to cancel this request?', [
      { text: 'No', style: 'cancel' },
      {
        ...
        onPress: () => {
          if (request) cancelRequest(request.id);
          dispatch({ type: 'CLEAR_REQUEST' });
    ...
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.center}>
          ...
          <TouchableOpacity style={styles.backButton} onPress={() => navigation.navigate('CellHome')}>
...
const styles = StyleSheet.create({
  ...
  iconCircle: { width: 160, height: 160, borderRadius: 80, backgroundColor: COLORS.success, alignItems: 'center', justifyContent: 'center', elevation: 8 },
  iconText: { color: COLORS.white, fontSize: 16, fontWeight: 'bold', textAlign: 'center', letterSpacing: 1 },
  infoCard: { backgroundColor: COLORS.white, borderRadius: 16, padding: 24, width: '100%', elevation: 4, marginBottom: 24 },
  infoTitle: { fontSize: 22, fontWeight: 'bold', color: COLORS.success, textAlign: 'center', marginBottom: 20 },
  ...
  cancelButton: { width: '100%', backgroundColor: COLORS.danger, borderRadius: 12, padding: 16, alignItems: 'center', elevation: 4 },
  cancelButtonText: { color: COLORS.white, fontSize: 16, fontWeight: 'bold' },
