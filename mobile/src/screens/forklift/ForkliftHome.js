...
import { connectSocket, registerAsForklift, declineRequest, returnFromLeave, getSocket } from '../../services/socket';
...
export default function ForkliftHomeScreen({ navigation }) {
  ...
  const initSocket = useCallback(async () => {
    ...
    // Force reset listener - fires when admin deletes this forklift or device (fix #1)
    socket.off('force_reset_' + deviceId);
    socket.on('force_reset_' + deviceId, () => {
      console.log('Force reset received - clearing forklift mode');
      handleForceReset();
    });

    socket.on('registered', () => console.log('Forklift registered successfully'));
    ...
    socket.on('request_taken', () => dispatch({ type: 'SET_INCOMING_REQUEST', payload: null }));
    ...
    Alert.alert('Confirm Leave', 'Going on leave: ' + comment.comment, [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: () => {
          ...
          if (incomingRequest) {
            declineRequest(incomingRequest.id, forkliftData.id, comment.comment);
            dispatch({ type: 'SET_INCOMING_REQUEST', payload: null });
          } else {
            const socket = getSocket();
  ...
  const getStatusText = () => !isConnected ? 'Disconnected' : isOnLeave ? 'On Leave' : state.currentRequest ? 'On Task' : 'Available';
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        <View style={[styles.statusCard, { borderColor: getStatusColor() }]}>
          ...
          <Text style={styles.statusCardSub}>
            {!isConnected ? 'Check your network connection' : isOnLeave ? 'You are currently on leave' : state.currentRequest ? 'Currently handling a task' : 'Waiting for incoming requests'}
          </Text>
        </View>
        ...
        {state.currentRequest && !isOnLeave ? (
          <View style={styles.taskCard}>
            ...
            <View style={styles.taskRow}>
              ...
              <Text style={styles.taskValue}>Cell {state.currentRequest.cell_number}</Text>
            </View>
            <View style={styles.taskRow}>
              <Text style={styles.taskLabel}>Type</Text>
              <Text style={styles.taskValue}>{state.currentRequest.forklift_type_name}</Text>
            </View>
            {state.currentRequest.operator_name ? (
              <View style={styles.taskRow}>
                <Text style={styles.taskLabel}>Operator</Text>
                <Text style={styles.taskValue}>{state.currentRequest.operator_name}</Text>
              </View>
            ) : null}
          </View>
        ) : null}
...
const styles = StyleSheet.create({
  ...
  modalCancelBtn: { marginTop: 16, padding: 14, backgroundColor: COLORS.lightGray, borderRadius: 8, alignItems: 'center' },
